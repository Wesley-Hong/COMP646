...
`{#| label: create a function} country_daily_revenues = function(data, selected_country='United Kingdom') {   data$days = as.character(data$days)   data |>     filter(country==selected_country) |>     select(country, year, month, days, revenue) |>     group_by(days) |>     summarize(daily_revenue=sum(revenue)) |>     mutate(days=parse_number(days)) |>     arrange(days) }`
